...
use web_spec::cli::{
    args::{Args, Commands},
    commands,
    output::{format_output, write_output},
};
...
async fn handle_run_command(
    ...
    if dry_run {
        ...
        let formatted = match format {
            ...
            _ => {
                ...
                if validation_result.is_valid() {
                    format!(
                        "✓ DRY RUN: Feature file would execute successfully\n  File: {}",
                        path_str
                    )
                } else {
                    format!(
                        "✗ DRY RUN: Feature file has validation errors\n  File: {}\n  Errors: {}\n  Warnings: {}",
                        path_str,
                        validation_result.error_count(),
                        validation_result.warning_count()
                    )
        ...
        return write_output(formatted, output_path).map_err(|e| anyhow::anyhow!(e));
    }
    ...
    let formatted = match format {
        ...
        "yaml" | "yml" => web_spec::execution::to_yaml_output(&result)
            .map_err(|e| anyhow::anyhow!("YAML serialization failed: {}", e))?,
        "tap" => web_spec::execution::to_tap_output(&result),
        "html" => web_spec::execution::to_html_output(&result),
        _ => web_spec::execution::to_text_output(&result),
    };

    // Write to file or stdout
    write_output(formatted, output_path).map_err(|e| anyhow::anyhow!(e))?;
...
async fn handle_validate_command(
    ...
    let output_text = match format {
        "json" => match commands::handle_validate_feature_json(feature) {
            Ok(json) => json,
            Err(e) => {
        ...
        "yaml" | "yml" => match commands::handle_validate_feature_yaml(feature) {
            Ok(yaml) => yaml,
            Err(e) => {
                eprintln!("Validation error: {}", e);
                std::process::exit(1);
        ...
        "tap" => match commands::handle_validate_feature_tap(feature) {
            Ok(tap) => tap,
            Err(e) => {
                eprintln!("Validation error: {}", e);
                std::process::exit(1);
        ...
        "html" => match commands::handle_validate_feature_html(feature) {
            Ok(html) => html,
            Err(e) => {
                eprintln!("Validation error: {}", e);
                std::process::exit(1);
    ...
    write_output(output_text, output_path).map_err(|e| anyhow::anyhow!(e))?;
    Ok(())
}
...
async fn handle_list_steps_command(
    ...
    match commands::handle_list_steps(category, search) {
        Ok(steps) => {
            ...
            let formatted =
                format_output(content, format, pretty).map_err(|e| anyhow::anyhow!(e))?;
            write_output(formatted, output_path).map_err(|e| anyhow::anyhow!(e))?;
            Ok(())
        }
        Err(e) => {
        ...
        Ok(json) => {
            let formatted = if format == "json" {
                if pretty {
                    ...
                    serde_json::from_str::<serde_json::Value>(&json)?.to_string()
                }
            } else {
                json
            };
...
async fn handle_search_steps_command(
    ...
    match commands::handle_search_steps(query, category) {
        Ok(results) => {
            ...
            let formatted =
                format_output(content, format, false).map_err(|e| anyhow::anyhow!(e))?;
            write_output(formatted, output_path).map_err(|e| anyhow::anyhow!(e))?;
            Ok(())
        }
        Err(e) => {
...
async fn handle_compare_command(
    ...
    let comparison: web_spec::execution::ComparisonResult = serde_json::from_str(&comparison_json)
    ...
    let formatted = match format {
        ...
        "yaml" | "yml" => {
            let yaml_value = serde_yaml::to_value(&comparison)
                .map_err(|e| anyhow::anyhow!("YAML serialization failed: {}", e))?;
            serde_yaml::to_string(&yaml_value).map_err(|e| anyhow::anyhow!("YAML error: {}", e))?
        }
        "html" => {
    ...
    for scenario in feature_file.scenarios.iter() {
        ...
        for (step_idx, step) in scenario.steps.iter().enumerate() {
            ...
            if debugger.should_pause(&scenario.name, &step.text) || auto_step {
                ...
                scenario_for_debug
                    .steps
                    .push(web_spec::execution::StepResult {
            ...
            if let Some(snapshot) = web_spec::execution::ExecutionSnapshot::from_scenario_step(
                &scenario_result,
                step_idx,
            ) {
        ...
        scenario_result.status = if all_passed {
            "passed".to_string()
        } else {
            "failed".to_string()
        };
...
async fn handle_batch_command(
    ...
    let result = executor.execute(&paths, &|feature_path| {
        ...
        let feature =
            parse_gherkin(&content).map_err(|e| format!("Failed to parse feature: {}", e))?;
    ...
    let formatted = match format {
        ...
        "yaml" | "yml" => executor.format_result(&result, "yaml"),
        _ => executor.format_result(&result, "text"),
    };
...
fn handle_alerts_command(
    ...
    // Write output
    write_output(final_output, output_path).map_err(|e| anyhow::anyhow!(e))?;
...
async fn main() -> anyhow::Result<()> {
    ...
    if let Some(command) = args.command {
        match command {
            ...
            Commands::Run { .. } => {
                eprintln!(
                    "Error: 'run' command requires the 'chromiumoxide-backend' feature to be enabled."
                );
                eprintln!(
                    "Please install with: cargo install web_spec --features chromiumoxide-backend"
                );
                std::process::exit(1);
            }
        }
