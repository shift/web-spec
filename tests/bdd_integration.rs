...
fn given_html_page_when_converted_to_markdown_then_output_is_non_empty() {
    ...
    assert!(
        markdown.contains("Test Title"),
        "Markdown should preserve headings"
    );
    assert!(
        markdown.contains("Test content"),
        "Markdown should preserve paragraphs"
    );
}
...
fn given_html_with_code_block_when_converted_then_code_is_preserved() {
    ...
    assert!(
        ...
        "Markdown should preserve code content"
    );
}
...
fn given_html_with_links_when_converting_then_links_are_preserved() {
    ...
    assert!(
        markdown.contains("[Click here]"),
        "Markdown should preserve link text"
    );
    assert!(
        markdown.contains("(https://example.com)"),
        "Markdown should preserve link URL"
    );
}
