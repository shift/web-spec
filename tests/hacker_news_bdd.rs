...
mod hacker_news_tests {
    ...
    async fn when_navigate_to_hacker_news(context: &mut HackerNewsContext, browser_type: BrowserType) {
        let mut browser = Browser::new(browser_type).await.expect(&format!("{} browser should initialize", context.backend_name));
    ...
    async fn when_navigate_chromiumoxide(context: &mut HackerNewsContext) {
        let mut browser = Browser::new_chromiumoxide().await.expect("Chromiumoxide browser should initialize");
        browser.navigate_to("https://news.ycombinator.com/news").await.expect("Navigation should succeed");
        browser.wait_for_load().await.expect("Wait should complete");
    ...
    fn then_can_extract_titles(context: &HackerNewsContext) {
        let html = context.html.as_ref().expect("HTML should be available");
        assert!(html.contains("Hacker News"), "HTML should contain Hacker News title");
    ...
    fn then_at_least_10_titles(context: &HackerNewsContext) {
        assert!(context.post_titles.len() >= 10, 
            "Should extract at least 10 post titles, got {}", context.post_titles.len());
    }
    ...
    fn then_each_title_has_text(context: &HackerNewsContext) {
        for (i, title) in context.post_titles.iter().enumerate() {
            assert!(!title.trim().is_empty(), 
                "Post title {} should not be empty", i + 1);
            assert!(title.len() > 5, 
                "Post title {} should be meaningful length, got: '{}'", i + 1, title);
        }
    }

    // THEN: Display top 10 titles
    fn then_display_top_10_titles(context: &HackerNewsContext) {
        println!("\n=== Top 10 Hacker News Posts ({}) ===", context.backend_name);
